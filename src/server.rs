use crate::client::{ChatCompletionRequest, ChatMessage, LlamaClient, StreamEvent};
use anyhow::Result;
...
impl ApiServer {
    pub fn new(client: Arc<LlamaClient>, port: u16) -> Self {
        Self { client, port }
    }
...
async fn handle_connection(mut socket: TcpStream, client: Arc<LlamaClient>) -> Result<()> {
    ...
    if method == "POST" && (path == "/v1/chat/completions" || path == "/chat/completions") {
        ...
        let messages: Vec<ChatMessage> = chat_req.messages.into_iter().map(|m| ChatMessage {
            role: m.role,
            content: Some(llama_rs::MessageContent::Text(m.content)),
            name: None,
            tool_call_id: None,
            tool_calls: None,
