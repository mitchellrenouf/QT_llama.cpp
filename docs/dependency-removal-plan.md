...
the operating-system APIs, and Qt may remain native build/runtime dependencies;
their Rust bindings must live in this repository.
...
tools, the CLI, and the Qt application must keep working unless a separate
change explicitly deprecates them.
...
| GUI | `qtrs` | Repository-owned minimal Qt C ABI shim and Rust FFI |
...
by tool schemas, JSONL, OpenAI-compatible requests, MCP, and GUI messages.
...
MCP child-process I/O, HTTP serving, and GUI bridging to owned worker threads
...
Implement strict WebSocket framing and handshake validation for the server/GUI
...
### 8. Replace the Qt Rust binding

Expose only the Qt operations MRML uses through a repository-owned C ABI shim,
then bind that ABI directly from Rust. Keep Qt object ownership on the C++ side
and use opaque handles across FFI. Match current UI smoke tests before removing
`qtrs`.

### 9. Lock the workspace
...
`serde_json`, `tokio`, templates, Chromium, or Qt; those need dedicated
