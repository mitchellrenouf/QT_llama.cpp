...
pub fn build_publickey_auth(session_id:&[u8],user:&str,key:&RsaPrivateKey)->Result<Vector<u8>,ProtocolError>{
    ...
    let public_blob=encode_public(&key.public)?;
...
fn encode_public(key:&RsaPublicKey)->Result<Vector<u8>,ProtocolError>{let mut writer=BinaryWriter::new();writer.string(b"ssh-rsa")?;write_mpint(&mut writer,&key.exponent)?;write_mpint(&mut writer,&key.modulus)?;Ok(writer.finish())}
...
#[cfg(test)]mod tests{use super::*;
 ...
 #[test]fn signatures_round_trip_through_ssh_blobs(){let key=key();let public=encode_public(&key.public).unwrap();let signature=sign_rsa_sha2_256(&key,b"message").unwrap();assert_eq!(verify_rsa_sha2_256(&public,b"message",&signature),Ok(()));assert_eq!(verify_rsa_sha2_256(&public,b"tampered",&signature),Err(ProtocolError::Authentication));}
