...
impl DynamicLibrary {
    ...
    /// Invoke the stable CUDA driver bootstrap ABI when this is `nvcuda.dll`.
    pub fn initialize_cuda_driver(&self) -> bool {
        type Initialize = unsafe extern "system" fn(u32) -> i32;
    ...
    /// Query the stable NVENC bootstrap ABI when this is `nvEncodeAPI64.dll`.
    pub fn nvenc_max_supported_version(&self) -> Option<u32> {
        type Query = unsafe extern "system" fn(*mut u32) -> i32;
