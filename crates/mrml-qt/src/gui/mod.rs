...
pub async fn launch_qt_gui(config: &Config) -> Result<()> {
    ...
    let port_file = std::env::temp_dir().join("mrml_agent_port");
    let _ = std::fs::write(&port_file, port.to_string());

    let agent = Arc::new(Mutex::new(MrmlAgent::new(config.clone())));
    ...
    println!(
        "🎨 Launching MRML native Qt6 interface [WS IPC port: {}]...",
        port
    );
    app::run(port)
}
