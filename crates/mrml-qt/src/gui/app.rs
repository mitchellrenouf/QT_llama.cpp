...
pub fn run(port: u16) -> anyhow::Result<()> {
    ...
    sidebar_layout.add(
        Label::new(
            "<div style='font-size:20px;font-weight:650;color:#f8fafc'>MRML</div>\
    ...
    input.borrow().set_placeholder_text("Message MRML…");
    ...
    let busy = Rc::new(RefCell::new(false));
    {
        ...
        send.connect_clicked(move || {
            ...
            *busy.borrow_mut() = true;
            ...
            status.borrow_mut().set_text("MRML is thinking…");
                ...
                while let Ok(e) = events.try_recv() {
                    match e["type"].as_str().unwrap_or("") {
                        ...
                        "turn_done" => {
                            ...
                            if !answer.is_empty() {
                                transcript.borrow_mut().push(("MRML".into(), answer));
                            }
    ...
    let help = Menu::new("Help")
        .action("About MRML", || {
            qtrs::messagebox::information(
                None,
                "MRML",
                "MRML\nMitchell Renouf Machine Learning Library and native Qt 6 AI agent.",
            )
        })
        .parent(&*window)
        .build();
...
fn welcome() -> String {
    "<div style='max-width:680px;margin:90px auto;color:#e5e7eb'>\
       ...
       <div style='font-size:15px;color:#94a3b8;line-height:1.55'>MRML is running locally. Ask a question, write some code, or load another GGUF model from Hugging Face.</div>\
     </div>".into()
}
fn render(items: &[(String, String)], live: &str) -> String {
    ...
    for (a, b) in items {
        let (accent, heading, alignment) = if a == "You" {
            ...
            ("#202633", "MRML", "margin-right:18%;margin-left:2%")
        };
        h.push_str(&format!(
    ...
    if !live.is_empty() {
        h.push_str(&format!(
            "<div style='margin-right:18%;margin-left:2%;margin-top:14px;padding:14px 16px;border-radius:12px;background:#202633;color:#f8fafc;line-height:1.55'>\
               <div style='font-size:12px;font-weight:600;color:#cbd5e1;margin-bottom:6px'>MRML · generating</div>{}</div>",
            live.replace('&', "&amp;")
                .replace('<', "&lt;")
                .replace('\n', "<br>")
        ));
