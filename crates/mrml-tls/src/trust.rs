...
pub fn verify_server_chain(hostname: &str, chain: &[&[u8]]) -> Result<(), CertificateError> {
    ...
    if trusted {
        ...
        Err(CertificateError::TrustStoreUnavailable)
    }
}
...
fn decode_base64(input: &[u8], output: &mut Vector<u8>) -> Result<(), CertificateError> {
    ...
    for byte in input.iter().copied() {
        ...
        used += 1;
        if used == 4 {
            let a = base64_value(group[0]).ok_or(CertificateError::Malformed)? as u32;
            let b = base64_value(group[1]).ok_or(CertificateError::Malformed)? as u32;
            let c = if group[2] == b'=' {
                0
            } else {
                base64_value(group[2]).ok_or(CertificateError::Malformed)? as u32
            };
            let d = if group[3] == b'=' {
                0
            } else {
                base64_value(group[3]).ok_or(CertificateError::Malformed)? as u32
            };
            output
                .try_push(((a << 2) | (b >> 4)) as u8)
                .map_err(|_| CertificateError::Malformed)?;
            if group[2] != b'=' {
                output
                    .try_push(((b << 4) | (c >> 2)) as u8)
                    .map_err(|_| CertificateError::Malformed)?;
            }
            if group[3] != b'=' {
                output
                    .try_push(((c << 6) | d) as u8)
                    .map_err(|_| CertificateError::Malformed)?;
            }
...
fn for_each_pem_certificate(
    ...
    let mut position = 0;
    while let Some(start) = find(&bundle[position..], BEGIN) {
        let content_start = position + start + BEGIN.len();
        let end =
            find(&bundle[content_start..], END).ok_or(CertificateError::Malformed)? + content_start;
        let mut der = Vector::new();
        decode_base64(&bundle[content_start..end], &mut der)?;
        if !visitor(&der) {
            break;
        }
        position = end + END.len();
    }
    Ok(())
}
#[allow(dead_code)]
...
mod tests {
    ...
    fn base64_decoder_is_strict() {
        ...
        decode_base64(b"TUlNRQ==", &mut out).unwrap();
        assert_eq!(&out[..], b"MIME");
        assert!(decode_base64(b"A===", &mut Vector::new()).is_err());
    }
}
