...
pub fn tool_error(message: impl core::fmt::Display) -> ToolError { mrml_error::message(message) }

#[derive(Debug, Clone)]
pub struct FunctionDefinition { pub name: Text, pub description: Text, pub parameters: Value }

#[derive(Debug, Clone)]
pub struct ToolDefinition { pub tool_type: Text, pub function: FunctionDefinition }

pub trait Tool: Send + Sync {
    fn name(&self) -> &str;
    fn description(&self) -> &str;
    fn parameters(&self) -> Value;
    fn execute(&self, workspace_root: &str, args: Value) -> impl Future<Output = Result<String>> + Send;
    fn to_tool_definition(&self) -> ToolDefinition {
        ToolDefinition { tool_type: "function".into(), function: FunctionDefinition {
            name: self.name().into(), description: self.description().into(), parameters: self.parameters(),
        }}
    }
}
...
impl<T: Tool> DynTool for T {
    ...
    fn parameters(&self) -> Value { Tool::parameters(self) }
    fn execute<'a>(&'a self, workspace_root: &'a str, args: Value) -> ToolFuture<'a> {
        unsafe { Pin::new_unchecked(Owned::new(Tool::execute(self, workspace_root, args))) }
    }
    fn to_tool_definition(&self) -> ToolDefinition { Tool::to_tool_definition(self) }
}

pub struct ToolRegistry { tools: Vector<Shared<dyn DynTool>> }

impl ToolRegistry {
    pub fn new() -> Self {
        let mut registry = Self { tools: Vector::new() };
    ...
    pub fn register(&mut self, tool: Shared<dyn DynTool>) {
        if let Some(existing) = self.tools.iter_mut().find(|existing| existing.name() == tool.name()) { existing.replace(tool); }
        else { self.tools.push(tool); }
    }
    pub fn get(&self, name: &str) -> Option<Shared<dyn DynTool>> { self.tools.iter().find(|tool| tool.name() == name).cloned() }
    pub fn definitions(&self) -> Vector<ToolDefinition> { self.tools.iter().map(|tool| tool.to_tool_definition()).collect() }
}
