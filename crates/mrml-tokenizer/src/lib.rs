...
pub struct Merge { pub left: u32, pub right: u32, pub token: u32 }

#[derive(Debug, Clone)]
pub struct Tokenizer { pieces: Vector<Vector<u8>>, merges: Vector<Merge> }
...
impl Tokenizer {
    pub fn byte_level() -> Self {
        ...
        for byte in 0..=255 { pieces.push(vector_from_slice(&[byte])); }
        Self { pieces, merges: Vector::new() }
    }
    pub fn vocab_size(&self) -> usize { self.pieces.len() }
    pub fn pieces(&self) -> &[Vector<u8>] { &self.pieces }
    pub fn merges(&self) -> &[Merge] { &self.merges }
    pub fn encode(&self, text: &str, boundaries: bool) -> Vector<u32> {
        ...
        if boundaries { tokens.push(BOS_ID); }
        tokens.extend(text.as_bytes().iter().map(|byte| *byte as u32 + BYTE_TOKEN_OFFSET));
        for merge in &self.merges { apply_merge(&mut tokens, *merge); }
        if boundaries { tokens.push(EOS_ID); }
        tokens
    }
    pub fn decode(&self, tokens: &[u32]) -> Vector<u8> {
        let capacity = tokens.iter().filter_map(|token| self.pieces.get(*token as usize)).map(Vector::len).sum();
        let mut bytes = Vector::with_capacity(capacity).unwrap();
        for token in tokens {
            if *token == BOS_ID || *token == EOS_ID { continue; }
            if let Some(piece) = self.pieces.get(*token as usize) { bytes.try_extend_from_slice(piece).unwrap(); }
        }
        bytes
    }
}

pub struct Trainer { documents: Vector<Vector<u32>>, tokenizer: Tokenizer }
...
impl Trainer {
    ...
    pub fn train(mut self, target_vocab_size: usize, minimum_frequency: u64) -> Tokenizer {
        ...
        while self.tokenizer.vocab_size() < target_vocab_size {
            ...
            for document in &mut self.documents { apply_merge(document, merge); }
        }
        self.tokenizer
    }
}
...
fn apply_merge(tokens: &mut Vector<u32>, merge: Merge) {
    ...
    while read < tokens.len() {
        if read + 1 < tokens.len() && tokens[read] == merge.left && tokens[read + 1] == merge.right {
            tokens[write] = merge.token; read += 2;
        } else { tokens[write] = tokens[read]; read += 1; }
        write += 1;
    }
...
mod tests {
    ...
    fn byte_fallback_round_trips_all_utf8() {
        ...
        assert_eq!(core::str::from_utf8(&tokenizer.decode(&tokenizer.encode(input, true))).unwrap(), input);
    }
    #[test]
    fn training_learns_frequent_pairs_and_shortens_text() {
        ...
        assert_eq!(core::str::from_utf8(&tokenizer.decode(&encoded)).unwrap(), "research research");
    }
}
