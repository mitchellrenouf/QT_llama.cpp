...
pub fn chacha20_block(key: &[u8; 32], counter: u32, nonce: &[u8; 12]) -> [u8; 64] {
    ...
    for index in 0..8 { initial[4 + index] = u32::from_le_bytes(key[index*4..index*4+4].try_into().expect("key word")); }
    for index in 0..3 { initial[13 + index] = u32::from_le_bytes(nonce[index*4..index*4+4].try_into().expect("nonce word")); }
    ...
    for index in 0..16 { output[index*4..index*4+4].copy_from_slice(&state[index].wrapping_add(initial[index]).to_le_bytes()); }
    output
}
...
pub fn chacha20_xor(key: &[u8; 32], nonce: &[u8; 12], mut counter: u32, data: &mut [u8]) -> bool {
    for chunk in data.chunks_mut(64) {
        ...
        for (byte, mask) in chunk.iter_mut().zip(block) { *byte ^= mask; }
        let Some(next) = counter.checked_add(1) else { return false; };
        counter = next;
    }
    true
}
...
mod tests {
    ...
    fn rfc_8439_block_vector() {
        ...
        assert_eq!(chacha20_block(&key,1,&nonce),decode("10f1e7e4d13b5915500fdd1fa32071c4c7d1f4c733c068030422aa9ac3d46c4ed2826446079faa0914c2d705d98b02a2b5129cd1de164eb9cbd083e8a2503c4e"));
    }
}
