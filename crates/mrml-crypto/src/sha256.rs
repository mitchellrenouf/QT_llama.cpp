...
impl Sha256 {
    ...
    fn compress(&mut self, block: &[u8; 64]) {
        ...
        for index in 0..64 {
            ...
            let choice = (e & f) ^ ((!e) & g);
            let first = h.wrapping_add(s1).wrapping_add(choice).wrapping_add(K[index]).wrapping_add(words[index]);
    ...
    pub fn finalize(mut self) -> [u8; 32] {
        ...
        for (bytes, word) in output.chunks_exact_mut(4).zip(self.state) {
...
mod tests {
    ...
    fn decode(hex: &str) -> [u8; 32] {
        let mut output = [0u8; 32];
        for (index, byte) in output.iter_mut().enumerate() { *byte = u8::from_str_radix(&hex[index*2..index*2+2], 16).unwrap(); }
        output
    }
    #[test]
    fn standard_vectors() {
        assert_eq!(Sha256::digest(b""), decode("e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855"));
        assert_eq!(Sha256::digest(b"abc"), decode("ba7816bf8f01cfea414140de5dae2223b00361a396177a9cb410ff61f20015ad"));
    }
    #[test]
    fn split_blocks_match_one_shot() {
        let mut hash = Sha256::new();
        hash.update(&[b'a'; 60]); hash.update(&[b'a'; 40]);
