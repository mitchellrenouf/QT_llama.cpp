...
pub use virtual_gpu::{
    ...
    Dispatch, DispatchId, DispatchTable, GPU_QUEUE_MESSAGE_BYTES, GpuBatchExecutor, GpuCommandRing,
    GpuCommandServiceError, GpuCompletion, GpuCompletionReceiver, GpuCompletionRing,
    GpuCompletionSender, GpuCompletionStatus, GpuDispatchBatch, GpuError, GpuHostBackend,
    GpuLifecycleError, GpuQueueReceiver, GpuQueueSender, GpuResourceBackend, GpuResourceOutcome,
    GpuResourceResponse, GpuResourceResponseReceiver, GpuResourceResponseSender, GpuRingConsumer,
    GpuRingProducer, GpuRingTicket, GpuSharedQueueLayout, GpuSharedRingIndices, GpuSubmitError,
    KernelId, MAX_ACTIVE_EXPERTS, MAX_BATCH_DISPATCHES, MAX_DISPATCH_BUFFERS, MAX_DISPATCH_SCALARS,
    MAX_GPU_CONTROL_BYTES, MAX_GPU_QUEUE_SLOTS, MediatedGpuExecutor, PreparedGpuBatch,
    PreparedGpuDispatch, ResourceCommand, ScalarArg, ScalarKind, ValidatedExpertSelection,
    ValidatedGpuBatch, ValidatedKernelLaunch, ValidatedMoeKernelLaunch, VerifiedGpuKernelBundle,
    VirtualGpuSession, process_gpu_resource_command, process_gpu_resource_command_with_response,
