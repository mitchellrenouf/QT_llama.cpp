...
impl Dispatch {
    ...
    pub fn validate_executor_schema(&self) -> Result<ValidatedKernelLaunch, GpuError> {
        match self.kernel.get() {
            ...
            5 => self.validate_geglu_q4_schema(true),
            6 => self.validate_geglu_q4_schema(false),
            ...
            15 => self.validate_prepare_ffn_schema(),
            16 => self.validate_finish_ffn_schema(),
    ...
    fn validate_geglu_q4_schema(&self, batched: bool) -> Result<ValidatedKernelLaunch, GpuError> {
        let expected_scalars = if batched { 3 } else { 2 };
        let expected_block = if batched { 256 } else { 128 };
        ...
        let gate_weights = self.accesses[0].ok_or(GpuError::InvalidKernelSchema)?;
        let up_weights = self.accesses[1].ok_or(GpuError::InvalidKernelSchema)?;
        let input = self.accesses[2].ok_or(GpuError::InvalidKernelSchema)?;
        ...
        let output_elements = rows
            .checked_mul(batch)
            .ok_or(GpuError::InvalidKernelSchema)?;
        let weight_ok = |access: BufferAccess| {
            access.mode == BufferMode::Read
                && access.offset.is_multiple_of(2)
                && access.length == weight_bytes
        };
        let expected_grid = if batched {
            [rows.div_ceil(16), batch.div_ceil(8), 1]
        } else {
            [rows.div_ceil(8), 1, 1]
        };
        if !weight_ok(gate_weights)
            || !weight_ok(up_weights)
            || !valid_f32_access(input, u64::from(input_elements) * 4, BufferMode::Read)
            || !valid_f32_access(output, u64::from(output_elements) * 4, BufferMode::Write)
            || self.grid != expected_grid
        {
    ...
    fn validate_embedding_q8_0_schema(&self) -> Result<ValidatedKernelLaunch, GpuError> {
        if self.access_count != 2
            || self.scalar_count != 3
            || self.shared_memory != 0
            || self.block != [256, 1, 1]
        ...
        let token = self.nonnegative_i32_scalar(0)?;
        let dimension = self.positive_i32_scalar(1)?;
        self.positive_f32_scalar(2)?;
        if dimension % 32 != 0 {
            return Err(GpuError::InvalidKernelSchema);
        }
        let row_bytes = u64::from(dimension / 32)
            .checked_mul(34)
            .ok_or(GpuError::InvalidKernelSchema)?;
        let output_bytes = u64::from(dimension)
            .checked_mul(4)
            .ok_or(GpuError::InvalidKernelSchema)?;
        if table.mode != BufferMode::Read
            || output.mode != BufferMode::Write
    ...
    fn validate_qkv_postprocess_schema(&self) -> Result<ValidatedKernelLaunch, GpuError> {
        if self.access_count != 5 || self.scalar_count != 10 || self.shared_memory != 0 {
            return Err(GpuError::InvalidKernelSchema);
        }
        let qkv = self.accesses[0].ok_or(GpuError::InvalidKernelSchema)?;
        let q_norm = self.accesses[1].ok_or(GpuError::InvalidKernelSchema)?;
        let k_norm = self.accesses[2].ok_or(GpuError::InvalidKernelSchema)?;
        let k_cache = self.accesses[3].ok_or(GpuError::InvalidKernelSchema)?;
        let v_cache = self.accesses[4].ok_or(GpuError::InvalidKernelSchema)?;
        let start_position = self.nonnegative_i32_scalar(0)?;
        let cache_start = self.nonnegative_i32_scalar(1)?;
        let heads = self.positive_i32_scalar(2)?;
        let kv_heads = self.positive_i32_scalar(3)?;
        let head_dimension = self.positive_i32_scalar(4)?;
        self.positive_f32_scalar(5)?;
        let batch = self.positive_i32_scalar(6)?;
        let cache_capacity = self.positive_i32_scalar(7)?;
        let k_format = self.nonnegative_i32_scalar(8)?;
        let v_format = self.nonnegative_i32_scalar(9)?;
        if head_dimension % 2 != 0
            || heads % kv_heads != 0
            || start_position
                .checked_add(batch - 1)
        ...
        let total_heads = kv_heads
            .checked_mul(2)
            .and_then(|value| value.checked_add(heads))
            .ok_or(GpuError::InvalidKernelSchema)?;
        let qkv_elements = total_heads
            .checked_mul(head_dimension)
            .and_then(|value| value.checked_mul(batch))
            .ok_or(GpuError::InvalidKernelSchema)?;
        let norm_bytes = u64::from(head_dimension) * 4;
        let cache_bytes = |format: u32| -> Result<u64, GpuError> {
            let bytes_per_head = match format {
                0 => u64::from(head_dimension) * 2,
                1 if head_dimension % 32 == 0 => u64::from(head_dimension / 32) * 34,
                2 if head_dimension % 32 == 0 => u64::from(head_dimension / 32) * 18,
                _ => return Err(GpuError::InvalidKernelSchema),
            };
            u64::from(cache_capacity)
                .checked_mul(u64::from(kv_heads))
                .and_then(|value| value.checked_mul(bytes_per_head))
                .ok_or(GpuError::InvalidKernelSchema)
        };
        let k_cache_bytes = cache_bytes(k_format)?;
        let v_cache_bytes = cache_bytes(v_format)?;
        let valid_cache = |access: BufferAccess, length: u64| {
            access.mode == BufferMode::Write
                && access.offset.is_multiple_of(2)
                && access.length == length
        };
        let expected_threads = if head_dimension >= 512 { 256 } else { 128 };
        if !valid_f32_access(qkv, u64::from(qkv_elements) * 4, BufferMode::ReadWrite)
            || !valid_f32_access(q_norm, norm_bytes, BufferMode::Read)
            || !valid_f32_access(k_norm, norm_bytes, BufferMode::Read)
            || !valid_cache(k_cache, k_cache_bytes)
            || !valid_cache(v_cache, v_cache_bytes)
            || self.grid != [total_heads, batch, 1]
            || self.block != [expected_threads, 1, 1]
        {
            return Err(GpuError::InvalidKernelSchema);
        }
        Ok(ValidatedKernelLaunch {
    ...
    fn validate_prepare_ffn_schema(&self) -> Result<ValidatedKernelLaunch, GpuError> {
        ...
        let elements = dimension
            .checked_mul(batch)
            .ok_or(GpuError::InvalidKernelSchema)?;
        let tensor_bytes = u64::from(elements) * 4;
        let vector_bytes = u64::from(dimension) * 4;
        for index in 0..2 {
            if !valid_f32_access(
                self.accesses[index].ok_or(GpuError::InvalidKernelSchema)?,
                tensor_bytes,
                BufferMode::Read,
            ) {
                return Err(GpuError::InvalidKernelSchema);
    ...
    fn validate_finish_ffn_schema(&self) -> Result<ValidatedKernelLaunch, GpuError> {
        ...
        let scale = self.scalars[0].ok_or(GpuError::InvalidKernelSchema)?;
        if scale.kind != ScalarKind::F32 || !f32::from_bits(scale.bits).is_finite() {
            return Err(GpuError::InvalidKernelSchema);
        }
        let dimension = self.positive_i32_scalar(1)?;
        let batch = self.positive_i32_scalar(2)?;
        ...
        let tensor_bytes = u64::from(elements) * 4;
        let vector_bytes = u64::from(dimension) * 4;
        let modes = [BufferMode::Read, BufferMode::Read, BufferMode::ReadWrite];
        ...
        if !valid_f32_access(
            self.accesses[6].ok_or(GpuError::InvalidKernelSchema)?,
            tensor_bytes,
            BufferMode::Write,
        ) || self.grid != [batch, 1, 1]
        {
...
mod tests {
    ...
    #[test]
    fn executor_binds_fused_q4_geglu_matrices() {
        let id = |slot| BufferId {
            slot,
            generation: 1,
        };
        let batched_accesses = [
            BufferAccess::new(id(0), 0, 288, BufferMode::Read),
            BufferAccess::new(id(1), 0, 288, BufferMode::Read),
            BufferAccess::new(id(2), 0, 1024, BufferMode::Read),
            BufferAccess::new(id(3), 0, 512, BufferMode::Write),
        ];
        let batched_scalars = [ScalarArg::i32(16), ScalarArg::i32(32), ScalarArg::i32(8)];
        let batched = Dispatch::new_with_scalars(
        ...
        let gemv_scalars = [batched_scalars[0], batched_scalars[1]];
        let gemv = Dispatch::new_with_scalars(
            KernelId::new(6).unwrap(),
            [2, 1, 1],
            [128, 1, 1],
            0,
            &gemv_accesses,
        ...
        .unwrap();
        assert_eq!(gemv.validate_executor_schema().unwrap().element_count(), 16);

        let short_up = [
            batched_accesses[0],
            BufferAccess::new(id(1), 0, 287, BufferMode::Read),
            batched_accesses[2],
            batched_accesses[3],
        ];
        let short_up = Dispatch::new_with_scalars(
            KernelId::new(5).unwrap(),
            [1, 1, 1],
            [256, 1, 1],
            0,
        ...
        .unwrap();
        assert_eq!(
            short_up.validate_executor_schema(),
            Err(GpuError::InvalidKernelSchema)
        );
    }
    ...
    #[test]
    fn executor_bounds_q8_embedding_rows_and_scale() {
        let accesses = [
            BufferAccess::new(
                BufferId {
                    slot: 0,
        ...
        let scalars = [
            ScalarArg::i32(9),
            ScalarArg::i32(64),
            ScalarArg::f32_bits(8.0f32.to_bits()),
        ];
        let dispatch = Dispatch::new_with_scalars(
            KernelId::new(27).unwrap(),
            [1, 1, 1],
            [256, 1, 1],
            0,
            &accesses,
        ...
        for invalid in [
            [ScalarArg::i32(10), scalars[1], scalars[2]],
            [scalars[0], ScalarArg::i32(63), scalars[2]],
            [
                scalars[0],
                scalars[1],
                ScalarArg::f32_bits(0.0f32.to_bits()),
            ],
        ] {
    ...
    #[test]
    fn executor_binds_ffn_preparation_and_completion_tensors() {
        let id = |slot| BufferId {
            slot,
            generation: 1,
        };
        let prepare_accesses = [
            BufferAccess::new(id(0), 0, 512, BufferMode::Read),
            BufferAccess::new(id(1), 0, 512, BufferMode::Read),
            BufferAccess::new(id(2), 0, 256, BufferMode::Read),
            BufferAccess::new(id(3), 0, 256, BufferMode::Read),
            BufferAccess::new(id(4), 0, 256, BufferMode::Read),
            BufferAccess::new(id(5), 0, 256, BufferMode::Read),
            BufferAccess::new(id(6), 0, 512, BufferMode::Write),
            BufferAccess::new(id(7), 0, 512, BufferMode::Write),
            BufferAccess::new(id(8), 0, 512, BufferMode::Write),
            BufferAccess::new(id(9), 0, 512, BufferMode::Write),
        ];
        let dimensions = [ScalarArg::i32(64), ScalarArg::i32(2)];
        let prepare = Dispatch::new_with_scalars(
            KernelId::new(15).unwrap(),
            [2, 1, 1],
            [512, 1, 1],
            0,
            &prepare_accesses,
        ...
        .unwrap();
        assert_eq!(
            prepare.validate_executor_schema().unwrap().element_count(),
            128
        );
        ...
        let finish_accesses = [
            BufferAccess::new(id(0), 0, 512, BufferMode::Read),
            BufferAccess::new(id(1), 0, 512, BufferMode::Read),
            BufferAccess::new(id(2), 0, 512, BufferMode::ReadWrite),
            BufferAccess::new(id(3), 0, 256, BufferMode::Read),
            BufferAccess::new(id(4), 0, 256, BufferMode::Read),
            BufferAccess::new(id(5), 0, 256, BufferMode::Read),
            BufferAccess::new(id(6), 0, 512, BufferMode::Write),
        ...
        let finish_scalars = [
            ScalarArg::f32_bits(1.0_f32.to_bits()),
            dimensions[0],
            dimensions[1],
        ];
        let finish = Dispatch::new_with_scalars(
            KernelId::new(16).unwrap(),
            [2, 1, 1],
            [512, 1, 1],
            0,
            &finish_accesses,
        ...
        .unwrap();
        assert_eq!(
            finish.validate_executor_schema().unwrap().element_count(),
            128
        );

        let mut wrong_mode = finish_accesses;
        wrong_mode[2] = BufferAccess::new(id(2), 0, 512, BufferMode::Write);
        let invalid = Dispatch::new_with_scalars(
            KernelId::new(16).unwrap(),
            [2, 1, 1],
            [512, 1, 1],
        ...
        let mut nonfinite = finish_scalars;
        nonfinite[0] = ScalarArg::f32_bits(f32::NAN.to_bits());
        let invalid = Dispatch::new_with_scalars(
            KernelId::new(16).unwrap(),
            [2, 1, 1],
            [512, 1, 1],
    ...
    #[test]
    fn executor_binds_qkv_postprocess_cache_formats_and_positions() {
        let id = |slot| BufferId {
            slot,
            generation: 1,
        };
        let accesses = [
            BufferAccess::new(id(0), 0, 4096, BufferMode::ReadWrite),
            BufferAccess::new(id(1), 0, 256, BufferMode::Read),
            BufferAccess::new(id(2), 0, 256, BufferMode::Read),
            BufferAccess::new(id(3), 0, 2048, BufferMode::Write),
            BufferAccess::new(id(4), 0, 1088, BufferMode::Write),
        ];
        let scalars = [
            ScalarArg::i32(0),
            ScalarArg::i32(0),
            ...
            ScalarArg::i32(2),
            ScalarArg::i32(64),
            ScalarArg::f32_bits(10_000.0_f32.to_bits()),
            ScalarArg::i32(2),
            ScalarArg::i32(8),
            ScalarArg::i32(0),
            ScalarArg::i32(1),
        ...
        let mut short_cache = accesses;
        short_cache[4] = BufferAccess::new(id(4), 0, 1087, BufferMode::Write);
        let invalid = Dispatch::new_with_scalars(
            KernelId::new(19).unwrap(),
            [8, 2, 1],
            [128, 1, 1],
        ...
        let mut invalid_format = scalars;
        invalid_format[9] = ScalarArg::i32(3);
        let invalid = Dispatch::new_with_scalars(
            KernelId::new(19).unwrap(),
            [8, 2, 1],
            [128, 1, 1],
        ...
        let mut overflowing_position = scalars;
        overflowing_position[0] = ScalarArg::i32(i32::MAX);
        let invalid = Dispatch::new_with_scalars(
            KernelId::new(19).unwrap(),
            [8, 2, 1],
            [128, 1, 1],
