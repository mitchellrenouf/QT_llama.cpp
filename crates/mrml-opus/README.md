...
narrowband, wideband, superwideband, and fullband CELT packets; and 10/20 ms
mono or stereo superwideband and fullband Hybrid packets. CELT analysis supports long and
...
Representative packets from every encoded SILK, CELT, and Hybrid
configuration have also
...
separation. Automatic CELT-to-Hybrid and, when its SILK packet budget permits,
CELT-to-SILK beginning redundancy is emitted. The allocation-free
one-frame-lookahead encoder emits end redundancy for automatic SILK/Hybrid-to-
CELT transitions. Independent interoperability-vector coverage remains
incomplete. SILK-only packets can emit caller-supplied beginning or end
...
payload, and both final ranges. Pass `--report-configs` to break the totals
down by TOC configuration. Pass `--require-exact` to make any decode or
